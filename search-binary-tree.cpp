/*************************************************************************
	> File Name: search-binary-tree.cpp
	> Author: Aiiii
	> Mail: 2646967818@qq.com
	> Created Time: Sat 19 Dec 2020 10:00:43 AM CST
 ************************************************************************/
...
void update_size(Node *root) {
    root->size = SIZE(root->lchild) + SIZE(root->lchild) + 1;
    return ;
}
...
int search_k(Node *root, int k) {
    ...
    if (SIZE(L(root)) - 1 == k) return root->key;
    if (k <= SIZE(L(root))) {
...
Node *erase(Node *root, int key) {
    ...
    if (key < root->key) {
        root->lchild = erase(root->rchild, key);
